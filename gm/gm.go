...
func main() {
	...
	// Load IP lists
	gIPs, err := loadIPs("g.txt")
	if err != nil {
		fmt.Printf("Error loading g.txt: %v\n", err)
		return
	}
	mIPs, err := loadIPs("m.txt")
	if err != nil {
		fmt.Printf("Error loading m.txt: %v\n", err)
		return
	}
	...
	outputFile := strings.TrimSuffix(*inputFile, ".csv") + "_gm_filtered.csv"
	...
	for {
		...
		if flowStatus == "ALLOWED" && (isIPInList(destIP, gIPs) || isIPInList(destIP, mIPs)) && !isPublicIP(sourceIP) && !isIPInList(sourceIP, knownIPs) {
